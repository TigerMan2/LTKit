...
@implementation TMStarView

- (instancetype)initWithFrame:(CGRect)frame delegate:(id<TMStarViewDelegate>)delegate
{
    self = [super initWithFrame:frame];
    if (self) {
        _numberOfStars = 5;
        _starStyle = TMStarViewStyleWholeStar;
        _currentScore = 0;
...
- (instancetype)initWithFrame:(CGRect)frame animated:(BOOL)animated numberOfStars:(NSInteger)numberOfStars currentScore:(CGFloat)score starStyle:(TMStarViewStyle)starStyle foredImageName:(nullable NSString *)foredImageName backImageName:(nullable NSString *)backImageName delegate:(id<TMStarViewDelegate>)delegate {
    self = [super initWithFrame:frame];
    if (self) {
...
- (instancetype)initWithFrame:(CGRect)frame animated:(BOOL)animated numberOfStars:(NSInteger)numberOfStars currentScore:(CGFloat)score starStyle:(TMStarViewStyle)starStyle foredImageName:(nullable NSString *)foredImageName backImageName:(nullable NSString *)backImageName finish:(finishBlock)finish {
    self = [super initWithFrame:frame];
    if (self) {
        _numberOfStars = numberOfStars;
        _currentScore = [self sumScore:score starStyle:starStyle];
...
- (CGFloat)sumScore:(CGFloat)currentScore starStyle:(TMStarViewStyle)starStyle {
    switch (starStyle) {
        case TMStarViewStyleWholeStar:
            return ceilf(currentScore);
        case TMStarViewStyleHalfStar:
            return roundf(currentScore) > currentScore ? ceilf(currentScore) : (ceilf(currentScore) - 0.5);
        case TMStarViewStyleIncompleteStar:
            return currentScore;
