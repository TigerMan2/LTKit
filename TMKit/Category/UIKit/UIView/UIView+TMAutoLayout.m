...
#import "UIView+TMAutoLayout.h"

@implementation UIView (TMAutoLayout)
...
- (NSLayoutConstraint *)addConstraintWithTopView:(UIView *)topView bottomView:(UIView *)bottomView constant:(CGFloat)constant {
    NSLayoutConstraint *layoutConstraint = [NSLayoutConstraint constraintWithItem:topView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:bottomView attribute:NSLayoutAttributeTop multiplier:1 constant:constant];
...
- (void)addConstraintCenterXToView:(UIView *)xView centerYToView:(UIView *)yView {
    if (xView) {
        [self addConstraint:[NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:xView attribute:NSLayoutAttributeCenterX multiplier:1 constant:0]];
    }
    if (yView) {
        [self addConstraint:[NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:yView attribute:NSLayoutAttributeCenterY multiplier:1 constant:0]];
    }
}
