...
#import "TMCyclePagerView.h"

NS_INLINE BOOL TMEqualIndexSection(TMIndexSection indexSection1, TMIndexSection indexSection2) {
    return (indexSection1.index == indexSection2.index && indexSection1.section == indexSection2.section);
}

NS_INLINE TMIndexSection TMMakeIndexSection(NSInteger index, NSInteger section) {
    TMIndexSection indexSection;
    indexSection.index = index;
    indexSection.section = section;
    return indexSection;
}

@interface TMCyclePagerView ()
<
    UICollectionViewDelegate,
    UICollectionViewDataSource,
    UICollectionViewDelegateFlowLayout,
    TMCyclePagerTransformLayoutDelegate
>
{
    struct {
...
@property (nonatomic, assign) TMIndexSection beginDragIndexSection;
...
@implementation TMCyclePagerView
...
- (void)setupSubviews {
    TMCyclePagerTransformLayout *layout = [[TMCyclePagerTransformLayout alloc] init];
...
- (void)timerFired:(NSTimer *)timer {
    ...
    [self scrollToNearlyIndexAtDirection:TMPagerScrollDirectionRight animated:YES];
}

#pragma mark  getter

- (TMCyclePagerViewLayout *)layout {
    if (!_layout) {
...
- (void)setDelegate:(id<TMCyclePagerViewDelegate>)delegate {
    _delegate = delegate;
    ...
    if (self.collectionView && self.collectionView.collectionViewLayout) {
        ((TMCyclePagerTransformLayout *)self.collectionView.collectionViewLayout).delegate = _delegateFlags.applyTransformToAttributes ? self : nil;
    }
}

- (void)setDataSource:(id<TMCyclePagerViewDataSource>)dataSource {
...
- (void)scrollToNearlyIndexAtDirection:(TMPagerScrollDirection)direction animated:(BOOL)animated {
    TMIndexSection indexSection = [self nearlyIndexPathAtDirection:direction];
...
- (void)scrollToItemAtIndex:(NSInteger)index animated:(BOOL)animated {
    ...
    if (!_isInfiniteLoop) {
        [self scrollToItemAtIndexSection:TMMakeIndexSection(index, 0) animated:animated];
        return;
    }
    [self scrollToItemAtIndexSection:TMMakeIndexSection(index, (index >= self.curIndex) ? _indexSection.section : _indexSection.section + 1) animated:animated];
}
...
- (void)updateLayout {
    ...
    ((TMCyclePagerTransformLayout *)_collectionView.collectionViewLayout).layout = self.layout;
}
...
- (BOOL)isValidIndexSection:(TMIndexSection)indexSection {
...
- (TMIndexSection)nearlyIndexPathAtDirection:(TMPagerScrollDirection)direction {
    return [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
}

- (TMIndexSection)nearlyIndexPathForIndexSection:(TMIndexSection)indexSection direction:(TMPagerScrollDirection)direction {
    if (indexSection.index < 0 || indexSection.index >= _numberOfItems) {
        return indexSection;
    }
    if (!_isInfiniteLoop) {
        if (direction == TMPagerScrollDirectionRight && indexSection.index == (_numberOfItems - 1)) {
            return _autoScrollInterval > 0 ? TMMakeIndexSection(0, 0) : indexSection;
        } else if (direction == TMPagerScrollDirectionRight) {
            return TMMakeIndexSection(indexSection.index + 1, 0);
        }
        
        if (indexSection.index == 0) {
            return _autoScrollInterval > 0 ? TMMakeIndexSection(_numberOfItems - 1, 0) : indexSection;
        }
        return _autoScrollInterval > 0 ? TMMakeIndexSection(indexSection.index - 1, 0) : indexSection;
    }
    
    if (direction == TMPagerScrollDirectionRight) {
        if (indexSection.index < _numberOfItems - 1) {
            return TMMakeIndexSection(indexSection.index + 1, indexSection.section);
        }
        if (indexSection.section >= kPagerViewMaxSectionCount - 1) {
            return TMMakeIndexSection(indexSection.index, kPagerViewMaxSectionCount - 1);
        }
        return TMMakeIndexSection(0, indexSection.section + 1);
    }
    ...
    if (indexSection.section <= 0) {
        return TMMakeIndexSection(indexSection.index, 0);
    }
    return TMMakeIndexSection(_numberOfItems - 1, indexSection.section - 1);
}

- (TMIndexSection)caculateIndexSectionWithOffsetX:(CGFloat)offsetX {
    if (_numberOfItems <= 0) {
        return TMMakeIndexSection(0, 0);
    }
    ...
    return TMMakeIndexSection(curIndex, curSection);
}

- (CGFloat)caculateOffsetXAtIndexSection:(TMIndexSection)indexSection {
    if (_numberOfItems == 0) {
        return 0;
    }
    UICollectionViewFlowLayout *layout = (UICollectionViewFlowLayout *)_collectionView.collectionViewLayout;
...
- (void)resetPagerViewAtIndex:(NSInteger)index {
    ...
    [self scrollToItemAtIndexSection:TMMakeIndexSection(index, _isInfiniteLoop ? kPagerViewMaxSectionCount/3 : 0) animated:NO];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([_delegate respondsToSelector:@selector(pagerView:didSelectedItemCell:atIndexSection:)]) {
        [_delegate pagerView:self didSelectedItemCell:cell atIndexSection:TMMakeIndexSection(indexPath.item, indexPath.section)];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    TMIndexSection newIndexSection = [self caculateIndexSectionWithOffsetX:scrollView.contentOffset.x];
    ...
    TMIndexSection indexSection = _indexSection;
    ...
    if (_delegateFlags.didScrollFromIndexToNewIndex && !TMEqualIndexSection(_indexSection, indexSection)) {
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    if (fabs(velocity.x) < 0.35 || !TMEqualIndexSection(_beginDragIndexSection, _indexSection)) {
        targetContentOffset->x = [self caculateOffsetXAtIndexSection:_indexSection];
        return;
    }
    TMPagerScrollDirection direction = TMPagerScrollDirectionRight;
    if ((scrollView.contentOffset.x < 0 && targetContentOffset->x <= 0) || (targetContentOffset->x < scrollView.contentOffset.x && scrollView.contentOffset.x < scrollView.contentSize.width - scrollView.frame.size.width)) {
        direction = TMPagerScrollDirectionLeft;
    }
    TMIndexSection indexSection = [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
...
#pragma mark  TMCyclePagerTransformLayoutDelegate
...
- (void)dealloc
{
    ...
    ((TMCyclePagerTransformLayout *)_collectionView.collectionViewLayout).delegate = nil;
}
