...
- (CGSize)contentSize {
    CGFloat width = (_indicatorViews.count - 1) * (_pageIndicatorSize.width + _pageIndicatorSpaing) + _currentPageIndicatorSize.width + _contentInset.left + _contentInset.right;
...
- (void)setPageIndicatorSize:(CGSize)pageIndicatorSize {
    if (CGSizeEqualToSize(pageIndicatorSize, _pageIndicatorSize)) {
        return;
    }
    _pageIndicatorSize = pageIndicatorSize;
    if (CGSizeEqualToSize(_currentPageIndicatorSize, CGSizeZero) ||
        (_currentPageIndicatorSize.width < pageIndicatorSize.width &&
         _currentPageIndicatorSize.height < pageIndicatorSize.height)) {
...
- (void)updateIndicatorViews {
    ...
    NSMutableArray *indicatorViews = _indicatorViews ? [_indicatorViews mutableCopy] : @[].mutableCopy;
    if (indicatorViews.count < _numberOfPages) {
        ...
        for (NSInteger idx = indicatorViews.count; idx >= _numberOfPages; idx --) {
            ...
            [indicatorViews removeObject:indicatorView];
        }
    }
...
- (void)layoutIndicatorViews {
    ...
    switch (self.contentHorizontalAlignment) {
        ...
        case UIControlContentHorizontalAlignmentFill:
        {
            ...
            if (_indicatorViews.count > 1) {
                pageIndicatorSpaing = (CGRectGetWidth(self.frame) - _contentInset.left - _contentInset.right - _currentPageIndicatorSize.width - (_indicatorViews.count - 1) * _pageIndicatorSize.width) / (_indicatorViews.count - 1);
            }
        }
            break;
    ...
    for (UIImageView *indicatorView in _indicatorViews) {
        ...
        CGSize size = (_currentPage == index) ? _currentPageIndicatorSize : _pageIndicatorSize;
