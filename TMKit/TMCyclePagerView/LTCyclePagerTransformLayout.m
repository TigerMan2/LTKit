...
typedef NS_ENUM(NSUInteger, LTTransformLayoutItemDirection) {
    LTTransformLayoutItemLeft,
    LTTransformLayoutItemCenter,
    LTTransformLayoutItemRight,
};

@interface LTCyclePagerTransformLayout () {
    struct {
...
@interface LTCyclePagerViewLayout ()
@property (nonatomic, weak) UIView *pageView;
@end

@implementation LTCyclePagerTransformLayout
...
- (void)setDelegate:(id<LTCyclePagerTransformLayoutDelegate>)delegate {
    _delegate = delegate;
...
- (void)setLayout:(LTCyclePagerViewLayout *)layout {
    _layout = layout;
...
- (LTTransformLayoutItemDirection)directionWithCenterX:(CGFloat)centerX {
    LTTransformLayoutItemDirection direction = LTTransformLayoutItemRight;
    ...
    if (ABS(centerX - contentCenterX) < 0.5) {
        direction = LTTransformLayoutItemCenter;
    } else if ((centerX - contentCenterX) < 0) {
        direction = LTTransformLayoutItemLeft;
    }
    return direction;
}
...
- (BOOL)shouldInvalidateLayoutForBoundsChange:(CGRect)newBounds {
    return _layout.layoutType == LTCyclePagerTransformLayoutNormal ? [super shouldInvalidateLayoutForBoundsChange:newBounds] : YES;
}

- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    if (_delegateFlags.applyTransformToAttributes || _layout.layoutType != LTCyclePagerTransformLayoutNormal) {
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_delegateFlags.initializeTransformAttributes) {
        [_delegate pagerViewTransformLayout:self initializeTransformAttributes:attributes];
    }else if(_layout.layoutType != LTCyclePagerTransformLayoutNormal){
...
- (void)initializeTransformAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(LTCyclePagerTransformLayoutType)layoutType {
    switch (layoutType) {
        case LTCyclePagerTransformLayoutLinear:
            [self applyLinearTransformToAttributes:attributes scale:_layout.minimumScale alpha:_layout.minimumAlpha];
            break;
        case LTCyclePagerTransformLayoutCoverflow:
...
- (void)applyTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(LTCyclePagerTransformLayoutType)layoutType {
    switch (layoutType) {
        case LTCyclePagerTransformLayoutLinear:
...
- (void)applyLinearTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes scale:(CGFloat)scale alpha:(CGFloat)alpha {
    ...
    if (_layout.adjustSpacingWhenScroling) {
        LTTransformLayoutItemDirection direction = [self directionWithCenterX:attributes.center.x];
        CGFloat translate = 0;
        switch (direction) {
            case LTTransformLayoutItemLeft:
                translate = 1.15 * attributes.size.width * (1-scale)/2;
                break;
            case LTTransformLayoutItemRight:
                translate = -1.15 * attributes.size.width * (1-scale)/2;
                break;
...
@implementation LTCyclePagerViewLayout
