...
typedef NS_ENUM(NSUInteger, TMTransformLayoutItemDirection) {
    TMTransformLayoutItemLeft,
    TMTransformLayoutItemCenter,
    TMTransformLayoutItemRight,
};

@interface TMCyclePagerTransformLayout () {
    struct {
...
@interface TMCyclePagerViewLayout ()
@property (nonatomic, weak) UIView *pageView;
@end

@implementation TMCyclePagerTransformLayout
...
- (void)setDelegate:(id<TMCyclePagerTransformLayoutDelegate>)delegate {
    _delegate = delegate;
...
- (void)setLayout:(TMCyclePagerViewLayout *)layout {
    _layout = layout;
...
- (TMTransformLayoutItemDirection)directionWithCenterX:(CGFloat)centerX {
    TMTransformLayoutItemDirection direction = TMTransformLayoutItemRight;
    ...
    if (ABS(centerX - contentCenterX) < 0.5) {
        direction = TMTransformLayoutItemCenter;
    } else if ((centerX - contentCenterX) < 0) {
        direction = TMTransformLayoutItemLeft;
    }
    return direction;
}
...
- (BOOL)shouldInvalidateLayoutForBoundsChange:(CGRect)newBounds {
    return _layout.layoutType == TMCyclePagerTransformLayoutNormal ? [super shouldInvalidateLayoutForBoundsChange:newBounds] : YES;
}

- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    if (_delegateFlags.applyTransformToAttributes || _layout.layoutType != TMCyclePagerTransformLayoutNormal) {
        ...
        for (UICollectionViewLayoutAttributes *attributes in attributesArray) {
            ...
            } else if (_delegateFlags.initializeTransformAttributes) {
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_delegateFlags.initializeTransformAttributes) {
        [_delegate pagerViewTransformLayout:self initializeTransformAttributes:attributes];
    }else if(_layout.layoutType != TMCyclePagerTransformLayoutNormal){
...
- (void)initializeTransformAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(TMCyclePagerTransformLayoutType)layoutType {
    switch (layoutType) {
        case TMCyclePagerTransformLayoutLinear:
            [self applyLinearTransformToAttributes:attributes scale:_layout.minimumScale alpha:_layout.minimumAlpha];
            break;
        case TMCyclePagerTransformLayoutCoverflow:
            [self applyLinearTransformToAttributes:attributes scale:_layout.maximumAngle alpha:_layout.minimumAlpha];
            break;
...
- (void)applyTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(TMCyclePagerTransformLayoutType)layoutType {
    switch (layoutType) {
        case TMCyclePagerTransformLayoutLinear:
...
- (void)applyLinearTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes scale:(CGFloat)scale alpha:(CGFloat)alpha {
    ...
    if (_layout.adjustSpacingWhenScroling) {
        TMTransformLayoutItemDirection direction = [self directionWithCenterX:attributes.center.x];
        CGFloat translate = 0;
        switch (direction) {
            case TMTransformLayoutItemLeft:
                translate = 1.15 * attributes.size.width * (1-scale)/2;
                break;
            case TMTransformLayoutItemRight:
                translate = -1.15 * attributes.size.width * (1-scale)/2;
                break;
...
@implementation TMCyclePagerViewLayout
...
- (UIEdgeInsets)lastSectionInset {
    if (_itemVerticalCenter) {
        CGFloat verticalSpace = (CGRectGetHeight(_pageView.frame) - _itemSize.height)/2;
        return UIEdgeInsetsMake(verticalSpace, _sectionInset.left, verticalSpace, _sectionInset.right);
    }
    return UIEdgeInsetsMake(_sectionInset.top, _sectionInset.left, _sectionInset.right, _itemSpacing);
}
